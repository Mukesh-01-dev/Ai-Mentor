...
const Sidebar = ({ activePage = "dashboard" }) => {
  ...
  // On mobile: always show text. On desktop: respect sidebarCollapsed
  const showText = isMobile ? true : !sidebarCollapsed;
  const showIconsOnly = !isMobile && sidebarCollapsed;
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* Navigation */}
        <nav className={`mt-6 px-3 h-[calc(100vh-14rem)] scrollbar-hide ${showIconsOnly ? "overflow-visible" : "overflow-y-auto"}`}>
            ...
            {navigationItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {showText && (
                    <span
                      className={`ml-3 text-xs font-black uppercase tracking-tight ${isActive ? "text-white" : ""}`}
        ...
        {/* Bottom profile */}
        <div className="absolute bottom-4 left-0 right-0 px-3" ref={profileRef}>
          {profilePopupOpen && (
            <div className={`absolute bottom-full mb-4 left-3 right-3 bg-card/95 backdrop-blur-2xl border border-border/50 rounded-[2rem] shadow-[0_-20px_80px_rgba(0,0,0,0.3)] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300 z-90 ${showIconsOnly ? "w-48 -left-2" : ""}`}>
              <div className="p-4 border-b border-border/50 text-center">
                <img
                  ...
                  className="w-12 h-12 rounded-2xl mx-auto mb-2 shadow-xl border-2 border-card object-cover"
                  alt="User"
                  onError={(e) => {
                ...
                />
                <h4 className="text-[10px] font-black text-main uppercase tracking-tighter">{displayName}</h4>
              </div>
              <div className="p-1.5">
                <button
                  onClick={() => { navigate("/settings"); setProfilePopupOpen(false); }}
                  className="flex items-center w-full px-3 py-2.5 text-[10px] font-black uppercase text-main hover:bg-teal-500 hover:text-white rounded-2xl transition-all"
                >
                ...
                <button
                  ...
                  className="flex items-center w-full px-3 py-2.5 text-[10px] font-black uppercase text-red-500 hover:bg-red-500 hover:text-white rounded-2xl transition-all mt-1"
                >
          ...
          >
            <div className={`bg-card dark:bg-[#0a0f1e] rounded-[1.5rem] transition-all duration-300 ${showIconsOnly ? 'p-1' : 'p-3 flex items-center'}`}>
              <img
                src={user?.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${encodeURIComponent(`${user?.firstName || ""} ${user?.lastName || ""}`.trim() || user?.name || displayName)}`}
                className={`${showIconsOnly ? 'w-10 h-10' : 'w-8 h-8'} rounded-xl shadow-md border-2 border-white dark:border-slate-800 transition-all object-cover`}
                alt="Avatar"
                onError={(e) => {
                  const seed = encodeURIComponent(`${user?.firstName || ""} ${user?.lastName || ""}`.trim() || user?.name || displayName);
                  e.target.src = `https://api.dicebear.com/8.x/initials/svg?seed=${seed}`;
              ...
              {showText && (
                <div className="ml-2.5 flex-1 min-w-0">
                  <div className="text-[10px] font-black truncate uppercase tracking-tight" style={{ color: '#a3a2a3' }}>
                    {displayName}
                  </div>
                  <div className="text-[8px] font-bold uppercase tracking-widest mt-0.5" style={{ color: '#a3a2a3' }}>
                    {t("nav.account")}
                  </div>
                </div>
              )}
