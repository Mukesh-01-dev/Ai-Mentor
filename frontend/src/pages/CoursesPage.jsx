...
const CoursesPage = () => {
  ...
  /* ================= ACTIVE TAB FROM LOCATION STATE ================= */
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/payment/create-checkout-session`,
        {
          ...
          body: JSON.stringify({
            course: { id: selectedCourse.id, title: selectedCourse.title, priceValue },
          }),
        }
      );
  ...
  const handleReportSubmit = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/coures-reports`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          ...
          courseName: reportCourse || "General",
        }),
      });
      ...
      setReportText("");
      setReportType("Select Issue Type");
      setSubType("");
      setReportCourse("");
    } catch (err) {
      console.error(err);
  ...
  const filteredMyCourses = applyFilters(baseSearch(myCourses));
  ...
  return (
    <>
        ...
        <div className="relative z-10 max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5 text-center sm:text-left">
            <img
              src={avatarSrc}
              alt="Profile"
              className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full border-3 border-white/80 object-cover shadow-lg ${!user?.avatar_url && !(user?.isGoogleUser || !!user?.googleId)
                ? "p-2 sm:p-3 bg-white/20"
                : ""
                }`}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-fr">
              ...
              {filteredMyCourses.map((course) => {
                ...
                return (
                  <div
                    key={course.id}
                    className="bg-card rounded-3xl border border-border overflow-hidden shadow-sm flex flex-col h-full w-full"
                  >
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2 md:hidden">
                  <button
                    onClick={scrollLeft}
                    className="p-2 rounded-full bg-card border border-border hover:bg-teal-500/10 hover:border-teal-500/50 transition-all shadow-sm"
                  >
              ...
              >
                ...
                {filteredExploreCourses.map((course) => (
                  <div
                    key={course.id}
                    className="min-w-[220px] sm:min-w-[260px] md:min-w-[280px] snap-start bg-card rounded-3xl border border-border overflow-hidden shadow-sm flex flex-col"
                  >
                    <div className="relative h-32 sm:h-40">
                      <img
                        src={course.image}
      ...
      <ReportModal
        ...
        setSelectedCourse={setReportCourse}
