    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-[18px] font-bold text-main font-[Inter]">
            Settings
          </h2>
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-full bg-canvas-alt hover:bg-border transition-colors"
          >
        ...
        >
          {NAV_KEYS.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isDanger ? "bg-red-50 dark:bg-red-900/20" : "bg-teal-50 dark:bg-teal-900/20"}`}
                >
                  <Icon
                    className={`w-5 h-5 ${isDanger ? "text-red-500" : "text-[#00BEA5]"}`}
                  />
                </div>
                <span
                  className={`flex-1 font-semibold text-[15px] font-[Inter] ${isDanger ? "text-red-500" : "text-main"}`}
                >
                  {t(item.labelKey)}
                </span>
...
function MobileModal({ open, onBack, onClose, title, children }) {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div
        className="flex items-center gap-3 px-4 border-b border-border bg-card shadow-sm flex-shrink-0"
        style={{
      ...
      >
        <button
          onClick={onBack}
          className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-canvas-alt transition-colors active:scale-90"
        >
          <ArrowLeft className="w-5 h-5 text-main" />
        </button>
        <h2 className="text-[17px] font-bold text-main font-[Inter] flex-1 truncate">
          {title}
        </h2>
        <button
          onClick={onClose}
...
export default function Settings() {
  ...
  const [pageLoading, setPageLoading] = useState(false); //here is made change for continuous loading
    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-card rounded-2xl sm:rounded-[24px] shadow-[0_4px_6px_0_rgba(0,0,0,0.10),0_10px_15px_0_rgba(0,0,0,0.10)] p-4 sm:p-5 md:p-6">
              ...
              <img
                src={
                  avatarFile
                    ...
                    : user?.avatar_url
                      ? user.avatar_url
                      : `https://api.dicebear.com/8.x/initials/svg?seed=${encodeURIComponent(`${formData.firstName} ${formData.lastName}`)}`
                }
                onError={(e) => {
          ...
          <div className="flex-1 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-[42px] px-4 rounded-xl border border-border text-[16px] font-[Inter] focus:ring-2 focus:ring-primary focus:border-primary bg-input text-main"
                />
              </div>
              <div className="relative">
                <label className="absolute -top-2 left-4 bg-card px-2 text-[14px] text-muted font-medium font-[Inter]">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                className="w-full min-h-[80px] px-4 py-3 rounded-xl border border-border text-[16px] font-[Inter] resize-none focus:ring-2 focus:ring-primary focus:border-primary bg-input text-main"
              />
            </div>
          </div>
        </div>
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t border-border">
          <button
            type="button"
            className="h-[42px] px-6 rounded-xl border border-border bg-card text-main text-[16px] font-medium font-[Inter] hover:bg-canvas-alt"
          >
            {t("common.cancel")}
          </button>
          <button
            ...
            disabled={loading}
            className="h-[42px] px-6 rounded-xl bg-[#00BEA5] text-white text-[16px] font-medium font-[Inter] hover:opacity-90 disabled:opacity-50"
          >
      ...
      <div className="bg-card rounded-2xl sm:rounded-[24px] shadow p-4 sm:p-6 md:p-8">
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:gap-4 pt-6 border-t border-border mt-6">
          ...
          <button
            ...
            disabled={loading}
            className="h-[50px] px-6 rounded-xl bg-[#00BEA5] text-white hover:opacity-90 disabled:opacity-50"
          >
            {loading ? t("common.saving") : t("common.save_changes")}
          </button>
        </div>
      ...
      <div className="bg-card rounded-2xl sm:rounded-[24px] shadow-[0_4px_6px_0_rgba(0,0,0,0.10),0_10px_15px_0_rgba(0,0,0,0.10)] p-4 sm:p-6 md:p-8">
        <div className="space-y-6">
          {[
            {
              key: "twoFactorAuth",
              labelKey: "settings.security.two_factor",
              descKey: "settings.security.two_factor_desc",
            },
            {
              key: "loginAlerts",
              labelKey: "settings.security.login_alerts",
              descKey: "settings.security.login_alerts_desc",
            },
          ].map((item) => (
            <div
              key={item.key}
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-4 pt-6 border-t border-border mt-6">
          ...
          <button
            ...
            disabled={loading}
            className="h-[50px] px-6 rounded-xl bg-[#00BEA5] text-white text-[16px] font-medium font-[Inter] hover:opacity-90 disabled:opacity-50"
          >
            {loading ? t("common.saving") : t("common.save_changes")}
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={loading}
              className="h-[50px] px-8 rounded-xl bg-[#00BEA5] text-white font-medium hover:opacity-90 disabled:opacity-50"
            >
  ...
  return (
    <>
      ...
      <div className="lg:hidden flex items-center justify-between px-4 pt-5 pb-2">
        <h1 className="text-[22px] font-bold text-main font-[Inter]">
          Settings
        </h1>
        <button
        ...
        <div className="lg:hidden flex-1 px-4 pb-8 mt-4">
          <div className="bg-card rounded-2xl p-8 shadow-sm border border-border/40 text-center">
            ...
            <p className="text-xs text-muted">
              Tap "Open Settings" above to browse and edit your preferences.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[230] p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-8 w-full max-w-[520px] text-center shadow-xl">
            ...
            <p className="text-sm text-muted mb-6">
              Please type <span className="font-bold text-red-500">DELETE</span>{" "}
              to confirm.
            </p>
            <input
              type="text"
            ...
            <div className="flex justify-center gap-4">
              <button
                ...
                className="px-6 py-2 border rounded-lg text-main hover:bg-canvas-alt"
              >
                Cancel
              </button>
              <button
      ...
      {profilepopup && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[240] animate-fadeIn p-4">
          <div className="relative bg-gradient-to-br from-white to-slate-100 dark:from-slate-800 dark:to-slate-900 rounded-3xl p-6 sm:p-10 w-[90vw] max-w-[420px] text-center shadow-2xl border border-slate-200 dark:border-slate-700">
            ...
            <h2 className="text-2xl font-bold bg-gradient-to-r from-emerald-500 to-teal-500 bg-clip-text text-transparent mb-3">
              Profile Updated Successfully!
            </h2>
            <button
              onClick={() => setProfilePopup(false)}
              className="px-8 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-2xl font-semibold shadow-lg hover:scale-105 transition-all duration-300"
            >
              Ok
            </button>
          </div>
